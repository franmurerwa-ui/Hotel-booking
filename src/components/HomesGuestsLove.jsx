  ...
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 mt-8 sm:mt-10">
      <div className="flex items-center justify-between mb-3 sm:mb-4">
        <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900">Homes guests love</h2>
        <Link to="/search" className="text-[#0071c2] text-sm font-semibold hover:underline whitespace-nowrap shrink-0 ml-4">
          Discover homes →
        </Link>
      </div>
    ...
    <Link to={`/property/${p.id}`} className="group flex-shrink-0 w-[calc(50%-6px)] sm:w-52 lg:w-56 flex flex-col bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition">
      ...
      <div className="p-3 flex flex-col gap-1 flex-1">
        ...
        <div className="flex items-center gap-1.5 mt-0.5">
          ...
          <span className="text-xs text-gray-500 truncate">{p.reviews.toLocaleString()} reviews</span>
        </div>
